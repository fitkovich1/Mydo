...
const LoginPageForm = () => {
    ...
    const formikValuesArr = Object.keys(formik.values);

    return (
        <LoginForm onSubmit={formik.handleSubmit}>
            {formikValuesArr?.length >= 1 &&
            formikValuesArr.map((value) => {
                return (
                    <FormInput
                        id={value}
                        key={value}
            ...
            <LoginFormButton type="submit" disabled={isUserLoginLoading}>
